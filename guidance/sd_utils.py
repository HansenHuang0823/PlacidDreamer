...
class StableDiffusion(nn.Module):
    ...
    def train_step_BSD(self, text_embeddings, pred_rgb, guidance_ratio, pred_depth=None, pred_alpha=None,
        ...
        with torch.no_grad():
            ...
            tt = torch.cat([t.reshape(1, 1).repeat(latents.shape[0], 1).reshape(-1)] * 2, dim=0)
            ...
            if use_control_net:
                ...
                unet_output = self.unet(latent_model_input, tt, encoder_hidden_states=text_embeddings,
                                    ...
                                    mid_block_additional_residual=mid_block_res_sample, cross_attention_kwargs=cross_attention_kwargs).sample
            else:
                unet_output = self.unet(latent_model_input.to(self.precision_t), tt.to(self.precision_t), encoder_hidden_states=torch.cat([text_embeddings], dim=0).to(self.precision_t), cross_attention_kwargs=cross_attention_kwargs).sample
            ...
            noise_pred = noise_pred_uncond + 7.5 * classifier_guidance
        ...
        if iteration % guidance_opt.vis_interval == 0:   
            ...
            with torch.no_grad():
                pred_x0_latent_sp = pred_original(self.scheduler, noise_pred_uncond, t, latents_noisy)    
                pred_x0_latent_pos = pred_original(self.scheduler, noise_pred, t, latents_noisy) 
    ...
    def train_step_BSD_auto(self, text_embeddings, pred_rgb, pred_depth=None, pred_alpha=None,
                    grad_scale=1,use_control_net=False,
                    save_folder:Path=None, iteration=0, warm_up_rate = 0,
                    resolution=(512, 512), guidance_opt=None,as_latent=False, embedding_inverse = None):
        ...
        with torch.no_grad():
            ...
            denoise_guidance =  (noise_pred_uncond - noise) * 1.25
            noise_pred = noise_pred_uncond + 7.5 * classifier_guidance
        ...
        alpha_prod_t = self.scheduler.alphas_cumprod[t]
        w = torch.norm(noise_pred_uncond - noise) * torch.sqrt(1 - alpha_prod_t) / 50
        grad =  grad_ * w
    ...
    # def train_step_perpneg_SDS(self, text_embeddings, pred_rgb, pred_depth=None, pred_alpha=None,
    #                        grad_scale=1,use_control_net=False,
    ...
    #     classifier_guidance = delta_DSD.clone()
    #     denoise_guidance = 1.25 * (noise_pred_uncond - noise)
    #     def dot(tensor1, tensor2):
    #         return torch.sum(tensor1 * tensor2, dim=1)
    #     alpha = dot(denoise_guidance.flatten(1) - classifier_guidance.flatten(1), denoise_guidance.flatten(1)) / dot(denoise_guidance.flatten(1) - classifier_guidance.flatten(1), denoise_guidance.flatten(1) - classifier_guidance.flatten(1))
    #     alpha = torch.where(alpha < 0, 0, alpha)
    ...
    #         with torch.no_grad():
    #             pred_x0_latent_sp = pred_original(self.scheduler, noise_pred_uncond, t, latents_noisy)    
    #             pred_x0_latent_pos = pred_original(self.scheduler, noise_pred_post, t, latents_noisy)        
    #             pred_x0_pos = self.decode_latents(pred_x0_latent_pos.type(self.precision_t))
    #             pred_x0_sp = self.decode_latents(pred_x0_latent_sp.type(self.precision_t))
