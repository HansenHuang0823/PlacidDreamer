...
def dict_to_attr(d):
    return namedtuple('Attrs', d.keys())(*d.values())
...
def get_parser(**parser_kwargs):
    ...
    parser.add_argument('--model_path', type=str, default='stabilityai/stable-diffusion-2-1-base', help='Path to the model')
    ...
    parser.add_argument('--n_prompt', default="", type=str, nargs='+', help='negative prompt')
...
def main():
    ...
    if args.generation_mode == 'vsd':
        if args.phi_model == 'lora':
            ...
            vae_phi = vae
    elif args.generation_mode in ['sds', 'bsd']:
        unet_phi = None
        vae_phi = vae
    ...
    neg_input = tokenizer(
        ["unrealistic, blurry, low quality."] * max(args.particle_num_vsd,args.particle_num_phi), padding="max_length", max_length=max_length, return_tensors="pt"
    )
    with torch.no_grad():
    ...
    if args.generation_mode == 'bsd_neg':
        ...
        with torch.no_grad():
            neg_embeddings = text_encoder(negative_input.input_ids.to(device))[0]
            neg_embeddings = neg_embeddings.reshape(max(args.particle_num_vsd, args.particle_num_phi), len(args.n_prompt), neg_embeddings.shape[-2], neg_embeddings.shape[-1])
        text_embeddings_vsd = torch.cat([text_embeddings_vsd, neg_embeddings[:args.particle_num_vsd].transpose(0, 1).reshape(args.particle_num_vsd * len(args.n_prompt), neg_embeddings.shape[-2], neg_embeddings.shape[-1])])
        text_embeddings_phi = torch.cat([text_embeddings_phi, neg_embeddings[:args.particle_num_phi].transpose(0, 1).reshape(args.particle_num_vsd * len(args.n_prompt), neg_embeddings.shape[-2], neg_embeddings.shape[-1])])
    ...
    elif args.generation_mode in ['bsd']:
        cross_attention_kwargs = {'scale': args.lora_scale} if (args.generation_mode == 'vsd' and args.phi_model == 'lora') else {}
        # cnt = 0
        for step, chosen_t in enumerate(pbar):
            latents = get_latents(particles, vae, args.rgb_as_latents, use_mlp_particle=args.use_mlp_particle)
            ...
            latent_model_input = scheduler.scale_model_input(latent_model_input, t)
            with torch.no_grad():
                unet_output = unet(latent_model_input, torch.cat([t] * 2, dim=0), encoder_hidden_states=text_embeddings_vsd).sample
            noise_pred_uncond, noise_pred_text = unet_output.chunk(2)
            alpha_prod_t = scheduler.alphas_cumprod[t]

            classifier_guidance = (noise_pred_text - noise_pred_uncond)
            denoise_guidance = noise_pred_uncond * 0.018
            # if torch.dot(denoise_guidance.flatten(), classifier_guidance.flatten()).item() < 0:
            #     cnt += 1
            #     print(cnt)
            #     print(torch.dot(denoise_guidance.flatten(), classifier_guidance.flatten()))
            noise_pred = noise_pred_uncond + 7.5 * (noise_pred_text - noise_pred_uncond)

            alpha = torch.dot(denoise_guidance.flatten() - classifier_guidance.flatten(), denoise_guidance.flatten()) / torch.dot(denoise_guidance.flatten() - classifier_guidance.flatten(), denoise_guidance.flatten() - classifier_guidance.flatten())
            alpha = torch.zeros_like(alpha) if alpha.item() < 0 else alpha
            alpha = torch.ones_like(alpha) if alpha.item() > 1 else alpha
            grad_ = classifier_guidance + denoise_guidance - noise
            grad_ = alpha * classifier_guidance + (1 - alpha) * denoise_guidance


            w = alpha_prod_t ** 0.5

            target = (latents_vsd - w * grad_).detach()
            # d(loss)/d(latents) = latents - target = latents - (latents - grad) = grad
            loss = 0.5 * F.mse_loss(latents_vsd, target, reduction="mean") / args.batch_size
            loss.backward()
            optimizer.step()
