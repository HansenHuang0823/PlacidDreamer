...
class Zero123PipelineFor3D(DiffusionPipeline):
    r"""
    ...
    def prepare_multiview_inputs(self, image, elevation, embed_num=8, do_classifier_guidance=True):
        output = {}
        if do_classifier_guidance:
            if isinstance(image, PIL.Image.Image):
                output["image"] = [image] * (embed_num * 2)
            elif isinstance(image, torch.Tensor):
                output["image"] = torch.cat([image] * (embed_num * 2), dim=0)
            output["elevation"] = torch.Tensor([elevation - 30] * (embed_num * 2))
            azimuth_list = []
            for embed_id in range(embed_num):
                azimuth_list.append(360.0 / embed_num * embed_id)
            output["azimuth"] = torch.cat(azimuth_list* 2, dim=0)
            output["distance"] = torch.Tensor([0] * (embed_num * 2))
        else:
            if isinstance(image, PIL.Image.Image):
                output["image"] = [image] * embed_num
            elif isinstance(image, torch.Tensor):
                output["image"] = torch.cat([image] * embed_num, dim=0)
            output["elevation"] = torch.Tensor([elevation - 30] * embed_num)
            azimuth_list = []
            for embed_id in range(embed_num):
                azimuth_list.append(360.0 / embed_num * embed_id)
            output["azimuth"] = torch.cat(azimuth_list, dim=0)
            output["distance"] = torch.Tensor([0] * embed_num)
        return output        
    ...
    def __call__(
        ...
        num_init_steps = 1
        continue_steps = 0
        with self.progress_bar(total=num_init_steps) as progress_bar:
            step_image_list = []
            for i, t in enumerate(timesteps):
                ...
                embed_mask = torch.ones((8,)).cuda()

                # predict the noise residual
                if do_classifier_free_guidance:
                    # print(torch.cuda.memory_allocated(None) / 1024 **2)
                    model_output_uncond = self.unet(
                        latent_model_input[:8],
                        t,
                        encoder_hidden_states=image_embeddings[:8],
                        cross_attention_kwargs=cross_attention_kwargs,
                    )
                    noise_pred_uncond = model_output_uncond.sample
                    ...
                    noise_pred_text = model_output_cond.sample
                    ...
                    model_output = self.unet(
                        ...
                        cross_attention_kwargs=cross_attention_kwargs,
                        latent_plane_coarse=latent_plane_coarse,
                        latent_plane_coarse_kwargs=latent_plane_coarse_kwargs,
                        latent_plane_fine=latent_plane_fine,
                        latent_plane_fine_kwargs=latent_plane_fine_kwargs
                    )
                    noise_pred = model_output.sample
                if  i == num_init_steps - 1:
                    pred_x0 = self.scheduler.step(
                        noise_pred, t, latents, **extra_step_kwargs
                    ).pred_original_sample

                    latents = self.scheduler.add_noise(pred_x0, torch.randn_like(pred_x0), timesteps[continue_steps])
                else:
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                embed_mask = torch.ones((8,)).cuda()
                latent_plane_coarse_kwargs = {
                    "embed_mask": embed_mask.bool(),
                }
                latent_plane_fine_kwargs = {
                    "embed_mask": embed_mask.bool(),
                }
                if do_classifier_free_guidance:
                    # print(torch.cuda.memory_allocated(None) / 1024 **2)
                    ...
                    model_output_cond = self.unet(
                        ...
                        latent_plane_fine=latent_plane_fine,
                        latent_plane_fine_kwargs=latent_plane_fine_kwargs,
                        use_latent_plane=use_latent_plane
                    )
                    ...
                    if use_latent_plane:
                        vertice, faces, normals, values = marching_cubes(F.softplus(model_output_cond.voxel_sigma[0, :, :, :, 0]).flip([0]).cpu().numpy().transpose(0,2,1), level=15.0)
                        # vertice = vertice / (model_output_cond.voxel_sigma.shape[1] - 1) - 0.5
                        mesh = trimesh.Trimesh(vertice, faces, normals)
        ...
        if not return_dict:
            return (image, has_nsfw_concept), mesh
