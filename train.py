...
def get_pos_neg_text_embeddings(embeddings, azimuth_val, opt):
    if azimuth_val >= -90 and azimuth_val < 90:
        if azimuth_val >= 0:
            r = 1 - azimuth_val / 90
            end_z = embeddings['right side']
            which_side = 'right side'
        else:
            r = 1 + azimuth_val / 90
            end_z = embeddings['left side']
            which_side = 'left side'
        start_z = embeddings['front']
        # if random.random() < 0.3:
        #     r = r + random.gauss(0, 0.08)
        pos_z = r * start_z + (1 - r) * end_z
        text_z = torch.cat([pos_z, embeddings['front'], embeddings[which_side]], dim=0)
        if r > 0.8:
            front_neg_w = 0.0
        else:
        ...
        if azimuth_val >= 0:
            ...
            start_z = embeddings['right side']
            which_side = 'right side'
        else:
            r = 1 + (azimuth_val + 90) / 90
            start_z = embeddings['left side']
            which_side = 'left side'
        end_z = embeddings['back']
        # if random.random() < 0.3:
        #     r = r + random.gauss(0, 0.08)
        pos_z = r * start_z + (1 - r) * end_z
        text_z = torch.cat([pos_z, embeddings[which_side], embeddings['front']], dim=0)
...
def prepare_embeddings(guidance_opt, guidance):
    ...
    for d in ['front', 'left side', 'right side', 'back']:
...
def training(dataset, opt, pipe, gcams, guidance_opt, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from, save_video):
    ...
    for iteration in range(first_iter, opt.iterations + 1):        
        ...
        if args.auto_BSD:
            loss = guidance.train_step_BSD_auto(torch.stack(text_z_, dim=1), images,
            ...
            loss = guidance.train_step_BSD(torch.stack(text_z_, dim=1), images, args.lambda_,
                                    pred_depth=depths, pred_alpha=alphas,
                                    grad_scale=guidance_opt.lambda_guidance,
                                    use_control_net = use_control_net ,save_folder = save_folder,  iteration = iteration, warm_up_rate=warm_up_rate, 
                                    resolution=(gcams.image_h, gcams.image_w),
...
if __name__ == "__main__":
    ...
    parser.add_argument("--lora_path", type=str, default = None, help="The path to the lora checkpoints.")
    parser.add_argument("--name", type=str, default = None)
    parser.add_argument("--auto_BSD", type=bool, default=False, help="To use a variation of BSD including -epsilon as SDS. This can stably converge to satisfactory results.")
    parser.add_argument("--lambda_", type=float, default = 18.0, help="The ratio between classifier guidance and smoothing guidance, denoted as lambda in the final BSD expression.")
    ...
    if args.opt is not None:
        ...
        gp.LoRA_path = args.lora_path
        lp.workspace = args.name
