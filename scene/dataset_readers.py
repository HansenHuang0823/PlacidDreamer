...
def readCircleCamInfo(path,opt):
    ...
    if not os.path.exists(ply_path):
        ...
        if opt.init_shape == 'sphere':
            ...
            xyz[:, 1] = xyz[:, 1] * 0.6 - 0.3
            xyz[:, 2] = xyz[:, 2] * 1.2 - 0.6
        elif opt.init_shape == 'latent_plane':
            mesh = trimesh.load(opt.init_mesh)
            xyz = trimesh.sample.sample_surface(mesh, num_pts)[0]
            xyz = np.array(xyz)
            xyz_max = xyz.max(axis=0)
            xyz_min = xyz.min(axis=0)
            xyz = (xyz - xyz_min) / (xyz_max - xyz_min) * 1 - 0.5
            tmp_xyz = xyz.copy()
            xyz[:, [0, 1, 2]] = tmp_xyz[:, [0, 2, 1]]
            num_pts = xyz.shape[0]
